document.addEventListener("DOMContentLoaded", function () {
  ...
  function loadPerformances(page) {
    ...
    fetch(
      ...
      .then(function (data) {
        // "교육" 장르 필터링
        let educationPerformances = data.body.content.filter(function (
          performance
        ) {
          return performance.genre === "교육";
        });

        let pageCount =
          Number(data.body.pageable.pageNumber) *
          Number(data.body.pageable.pageSize);
        displayPerformances(pageCount, educationPerformances);
        totalPages = data.body.totalPages;
        console.log(educationPerformances);
      })
